...
| `karyawan.divisi`        | Kolom string bebas                                                                                 | Diganti menjadi `id_divisi` (FK → tabel `divisis`)                                                                                                                                                                                                                                                                                                                                                                 | Normalisasi data divisi; menjadi basis scoping approval cuti per divisi (`atasan_divisi`).                                                                                                                                                                                                    |
...
| Kuota cuti               | Tidak ada di ERD awal                                                                              | Ditambahkan tabel `tipe_cuti` (`nama_cuti`, `kuota_cuti`, `berlaku_untuk_status`, `is_active`) dan tabel `kuota_cuti_karyawan` (`id_karyawan`, `leave_type_id`, `year`, `quota`, `remaining_quota`) untuk menyimpan sisa kuota per karyawan per jenis cuti per tahun. Jenis cuti baku: **Cuti Tahunan** (12 hari, status Tetap), **Cuti Hari Raya** (4 hari, semua status), **Cuti Sakit** (6 hari, semua status). | Mendukung perhitungan sisa cuti otomatis (sesuai kebutuhan fungsional skripsi) dan jenis cuti yang dapat dikonfigurasi dengan kuota berbeda berdasarkan status karyawan.                                                                                                                      |
...
- FR-04.7 Sistem menghitung jumlah hari cuti otomatis (`Cuti::getJumlahHariAttribute`) dan mengelola sisa kuota cuti per jenis cuti (`kuota_cuti_karyawan`). Jenis cuti dan kuota baku yang berlaku adalah:

    | Jenis Cuti     | Kuota (hari/tahun) | Berlaku untuk Status           |
    | -------------- | ------------------ | ------------------------------ |
    | Cuti Tahunan   | 12                 | Tetap                          |
    | Cuti Hari Raya | 4                  | All (Tetap, Kontrak, Training) |
    | Cuti Sakit     | 6                  | All (Tetap, Kontrak, Training) |

    Kuota dikonfigurasi pada tabel `tipe_cuti` dan dapat diubah oleh Admin. Karyawan dengan status Kontrak/Training tidak mendapatkan Cuti Tahunan.
...
- FR-05.5 Jadwal kerja dapat dikaitkan dengan entitas `shifts` (kode shift, nama shift, jam masuk/pulang) untuk standarisasi jam kerja dan perhitungan otomatis keterlambatan/ketersediaan clock-out. Sistem mendefinisikan dua shift kerja baku:
    - **Shift Pagi** (`kode_shift: Pa`): jam masuk 08:00 — jam pulang 17:00.
    - **Shift Siang** (`kode_shift: Si`): jam masuk 13:00 — jam pulang 22:00.
...
- FR-09.2 HR/Admin dapat mengelola Shift (`shift.*`): kode shift, nama, jam masuk, jam pulang. Shift baku sistem:
    - **Shift Pagi** (`Pa`): 08:00–17:00.
    - **Shift Siang** (`Si`): 13:00–22:00.
...
- **karyawan**: `id_karyawan` (PK), `id_user` (FK, unik, 1:1), `nama`, `id_jabatan` (FK), `id_divisi` (FK), `id_shift` (FK), `tanggal_masuk`, `status_aktif`, `status_karyawan`, `face_embedding` (JSON, 128-dim vector), `face_image_path`.
- **shifts**: `id_shift` (PK), `kode_shift` (UK), `nama_shift`, `jam_masuk`, `jam_pulang`. Shift baku: Pagi (08:00–17:00), Siang (13:00–22:00).
- **tipe_cuti**: `id` (PK), `nama_cuti`, `kuota_cuti`, `berlaku_untuk_status`, `is_active`. Tipe baku: Cuti Tahunan (12 hr, Tetap), Cuti Hari Raya (4 hr, All), Cuti Sakit (6 hr, All).
- **kuota_cuti_karyawan**: `id` (PK), `id_karyawan` (FK), `leave_type_id` (FK → tipe_cuti), `year`, `quota`, `remaining_quota`.
...
- `divisis` 1:N `karyawan`; `jabatans` 1:N `karyawan`.
